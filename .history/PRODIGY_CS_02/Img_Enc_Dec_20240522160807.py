...
def encrypt_decrypt_image(operation):
    ...
    if image_path:
        ...
        if operation == "encrypt":
            ...
            manipulated_pixels = [(r+key) % 256 for r, g, b, *a in pixel_list for r, g, b in [(r+key)%256, (g+key)%256, (b+key)%256]]
            
            # Shuffle pixels
            random.shuffle(manipulated_pixels)
        else:  # decrypt
            # Unshuffle pixels
            shuffled_indices = list(range(len(pixel_list)))
            random.shuffle(shuffled_indices)
            unshuffled_pixels = [None] * len(pixel_list)
            for i, shuffled_index in enumerate(shuffled_indices):
                unshuffled_pixels[shuffled_index] = pixel_list[i]
            
            # Manipulate pixel values back
            manipulated_pixels = [(r-key) % 256 for r, g, b, *a in unshuffled_pixels for r, g, b in [(r-key)%256, (g-key)%256, (b-key)%256]]
