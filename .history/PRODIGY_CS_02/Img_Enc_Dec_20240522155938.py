    ...
    if image_path:
        ...
        for y in range(encrypted_image.height):
            for x in range(encrypted_image.width):
                ...
                g = (g + key) % 256
                b = (b + key) % 256
                if len(image.getpixel((x, y))) == 4:  # Check for alpha channel
    ...
    if image_path:
        ...
        for y in range(decrypted_image.height):
            for x in range(decrypted_image.width):
                r, g, b = encrypted_image.getpixel((x, y))[:3]  # Get RGB values
                r = (r - key) % 256
                g = (g - key) % 256
                b = (b - key) % 256
                if len(encrypted_image.getpixel((x, y))) == 4:  # Check for alpha channel
                    a = encrypted_image.getpixel((x, y))[3]
                    pixels[x, y] = (r, g, b, a)
                else:
                    pixels[x, y] = (r, g, b)
